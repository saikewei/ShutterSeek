...
func TestListLogs_NewestFirstAndPagination(t *testing.T) {
	...
	// Use login events (not throttled) to seed 5 rows for pagination
	for i := 0; i < 5; i++ {
		if err := svc.LogEvent(u.ID, u.Username, model.LogEventLogin, ""); err != nil {
			t.Fatalf("log: %v", err)
		}
...
func TestLogEvent_SessionThrottled(t *testing.T) {
	svc, u := setupLogTest(t)
	...
	// Immediate second session within the interval must be skipped
	if err := svc.LogEvent(u.ID, u.Username, model.LogEventSession, ""); err != nil {
		t.Fatalf("second session: %v", err)
	}
	// Login/logout are never throttled
	if err := svc.LogEvent(u.ID, u.Username, model.LogEventLogin, ""); err != nil {
		t.Fatalf("login: %v", err)
	}
	if err := svc.LogEvent(u.ID, u.Username, model.LogEventLogout, ""); err != nil {
		t.Fatalf("logout: %v", err)
	}

	var count int64
	svc.DB.Model(&model.UserLog{}).Where("user_id = ?", u.ID).Count(&count)
	if count != 3 { // session + login + logout
		t.Fatalf("expected 3 rows (throttled second session), got %d", count)
	}

	// A different user is not affected by the first user's limiter
	u2, err := svc.CreateUser("TEST_log_other", "secret123", "guest")
	if err != nil {
		t.Fatalf("create other user: %v", err)
	}
	defer func() {
		svc.DB.Exec("DELETE FROM user_logs WHERE user_id = ?", u2.ID)
		deleteUser(svc.DB, u2.ID)
	}()
	if err := svc.LogEvent(u2.ID, u2.Username, model.LogEventSession, ""); err != nil {
		t.Fatalf("other session: %v", err)
	}
	var c2 int64
	svc.DB.Model(&model.UserLog{}).Where("user_id = ?", u2.ID).Count(&c2)
	if c2 != 1 {
		t.Fatalf("other user session should log, got %d", c2)
	}
}
