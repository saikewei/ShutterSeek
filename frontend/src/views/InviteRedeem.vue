<template>
  <div class="min-h-screen flex items-center justify-center relative overflow-hidden bg-canvas">
    ...
    <div class="absolute -top-40 -left-40 w-[28rem] h-[28rem] rounded-full bg-accent/5 blur-3xl" />
    <div class="absolute -bottom-40 -right-40 w-[28rem] h-[28rem] rounded-full bg-accent/10 blur-3xl" />
    ...
    <div v-if="!checking" class="relative w-full max-w-sm px-6">
      <div class="bg-raised/60 backdrop-blur-xl border border-line rounded-2xl p-8 shadow-2xl shadow-black/50">
        <div class="flex flex-col items-center mb-8">
          ...
          <h1 class="text-xl font-semibold text-ink mt-4 tracking-tight">创建访客账号</h1>
          <p class="text-sm text-ink-3 mt-1">通过邀请码加入</p>
        </div>
      ...
      <p class="text-center text-xs text-ink-3 mt-6">Invited by the archive owner · 由相册主人邀请</p>
    </div>
  </div>
</template>
